...
ui <- page_sidebar(
   ...
   navset_card_tab(
     ...
     nav_panel(
       ...
       markdown("Olá! Aqui é a [Beatriz Milz](https://github.com/beatrizmilz), e este é um aplicativo Shiny que permite explorar a demanda dos cursos da ETEC para o <b>primeiro semestre de 2025</b>. <br> Você pode filtrar por período, unidade e curso, além de visualizar as informações em uma tabela e em um mapa interativo. <br> Algumas coordenadas das unidades podem estar incorretas, pois foram obtidas através de geolocalização usando a API do Google. Caso encontre algum erro, você pode me avisar pelo [GitHub](https://github.com/beatrizmilz/etec/issues/new) ou [email: milz.bea@gmail.com](mailto:milz.bea@gmail.com). <br> Obrigada! ")
     )
   )
...
server <- function(input, output) {
  ...
  output$tabela <- renderReactable({
    dados_filtrados() |>
      select(curso, periodo, inscritos, vagas, demanda, unidade) |>
  ...
  output$grafico <- plotly::renderPlotly({
    ...
    grafico_ggplot <- dados_prep |>
      ...
      labs(title = "Cursos com a maior demanda",
           x = "Pessoas inscritas / Vagas",
           y = "Cursos") +
      theme_minimal()
  ...
  output$mapa <- leaflet::renderLeaflet({
   dados_prep_unidade <- dados_filtrados() |>
      dplyr::group_by(unidade, lat, lon) |>
      dplyr::summarise(
        inscritos = sum(inscritos, na.rm = TRUE),
        vagas = sum(vagas, na.rm = TRUE),
   ...
   dados_prep_unidade |>
      ...
      leaflet::addCircleMarkers(
        lng = ~lon,
        lat = ~lat, clusterOptions = leaflet::markerClusterOptions(),
        popup = ~paste0(
          "<strong>Unidade:</strong> ", unidade, "<br>",
          "<strong>Cursos:</strong> ", cursos , "<br>",
          "<strong>Inscritos:</strong> ", inscritos, "<br>",
          "<strong>Vagas:</strong> ", vagas, "<br>"
        ),
      )
  })
